...
$all_query = DB::table('orders')->where('orders.caterer_id', $user_id)->whereRaw('delivery_date > DATE_ADD( NOW(), INTERVAL 2 HOUR)')->get();

$confirm_query = DB::table('orders')->where('orders.caterer_id', $user_id)->whereRaw('delivery_date > DATE_ADD( NOW(), INTERVAL 2 HOUR)')->where('status', 'Pending')->get();

$cancel_query = DB::table('orders')->where('orders.caterer_id', $user_id)->whereRaw('delivery_date > DATE_ADD( NOW(), INTERVAL 2 HOUR)')->where('status', 'Cancel')->get();
?>
        ...
        <div class="right_wrap_inner">
            <div class="informetion informetion_new">
                ...
                <div class="informetion_top">
                    <div class="tatils"> <span class="personal">Scheduled Orders</span></div>
                      <div class="tabel_option">
                        <ul class="tabel_option_li">
                            <li class="{{Request::is('order/scheduleorders/all') ? 'active' :''}}"><a href="{{HTTP_PATH.'order/scheduleorders/all'}}">ALL ({{$all_query ? count($all_query) :'0'}})</a></li>
                            <li class="{{Request::is('order/scheduleorders/schedule') ? 'active' :''}}"><a href="{{HTTP_PATH.'order/scheduleorders/schedule'}}">SCHEDUALED ({{$confirm_query ? count($confirm_query) :'0'}})</a></li>
                            <li class="{{Request::is('order/scheduleorders/cancel') ? 'active' :''}}"><a href="{{HTTP_PATH.'order/scheduleorders/cancel'}}">CANCELLED ({{$cancel_query ? count($cancel_query):'0'}})</a></li>
                        </ul>  
                    </div>
                    <div class="search-area">
                        {{ Form::open(array('url' => '/order/scheduleorders', 'method' => 'post', 'id' => 'adminAdd', 'files' => true,'class'=>'form-inline')) }}
...
<!--                                                <div data-title="Created" class="td_tables2">
                                                ...
                                                <div data-title="Action" class="td_tables2">
                                                    <div class="actions">
                                                        <?php
//                                                        echo html_entity_decode(HTML::link('order/receivedview/' . $data->slug . '/schedule', '<i class="fa fa-search"></i>', array('class' => 'btn btn-primary btn-xs', 'title' => 'View Order Details')));
                                                        ?>
                                                    ...
                                                    <div class="wrapper_login">
                                                        ...
                                                        ?>
                                                            ...
                                                            <div class="order_div">
                                                             ...
                                                             <div class="gradient"> <img src="{{ URL::asset('public/img/front') }}/loader.jpg"  alt=""></div>
                                                             
                                                             <div class="cover_before">
                                                            <?php
