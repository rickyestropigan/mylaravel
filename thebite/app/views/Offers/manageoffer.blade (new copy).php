                    ...
                    <div class="menu_block  <?php echo $class; ?>  order_box">
                        ...
                        <div class="address min_height"><?php
                            if ($record->all_menu == 1) {
                                echo 'All menu items with orders $ ' . $record->above_price . ' above';
                            } else {
                                echo 'On Select Menu Itemswith Orders Above $ ' . $record->above_price . '<br> (';
                        ...
                        <div class="time_calendar<?php echo $record->id; ?>" style="display:none;">
                            ...
                            foreach ($offer_slots as $offerslot) {
                                if (!empty($offerslot->color) && $offerslot->color != '#369F5C' && !in_array($offerslot->color, $clrcheck)) {
                                            ...
                                            if ($offerslot->status == 0) {
                                                ...
                                                <div class="boxx"> 
                                                    <div class="top_row"><label><?php echo date('g:i A', strtotime($offerslot->start_time)); ?> - <?php echo date('g:i A', strtotime($offerslot->end_time)); ?></label>
                                                        <!--<i class="fa fa-pencil edit_slot" data-offerid="<?php echo $offerslot->offer_id; ?>" data-id="<?php echo $offerslot->id; ?>" data-tex="Edit <?php echo $offerslot->discount; ?> % OFF on Time <?php echo date('g:i A', strtotime($offerslot->start_time)); ?> - <?php echo date('g:i A', strtotime($offerslot->end_time)); ?>" data-toggle="modal" data-target="#editOfferSlotModal"></i>-->
                                                    </div>   
...
<div id="editOfferSlotModal" class="modal fade editscreen" role="dialog">
...
<script>
    $(document).on('ready', function () {
        ...
        $('#mnbx').on('click', '.edit_slot', function () {

            var upid = $(this).data('id');
            var uptext = $(this).data('tex');
...
//            console.log($(this).closest('a'));
            ...
            $.ajax({
                url: "<?php echo HTTP_PATH; ?>offer/editofferslotpage",
                dataType: 'html',
                type: 'POST',
                cache: false,
                data: data,
                success: function (result) {
                    $('#editOfferSlotModal').html(result);
                    $("#editOfferSlotModal" + " .changecolor").data('obj', that);
                    $("#editOfferSlotModal" + " #offchange").data('obj', that);
                }
            });
        });
...
//            var modid = $(this).data('offerid');
//            var status = $(this).data('status');
//            console.log(modid);
//            $('#editcolorSlotModal' + modid).modal('show');
//            $("#editcolorSlotModal" + modid + " .changecolor").data('obj', this);
//            $("#editcolorSlotModal" + modid + " #offchange").data('obj', this);
//            $('#tmslot').val(upid);
//            $('#upslcolr').val(modid);
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.trim() != 'notchange') {
                        ...
                        $(that).find('div.peop span').text(offer + '% off')
                    } else
                    {
...
<script>
//    $(document).on('click', '#offchange', function () {
//        var upid = $(this).data('soltid');
//        var discount = $('#editslotModal #offerchange').val();
//        var color = $("#colorslott").val();
//        var that = $(this).data('obj');
//
...
//
//                    if (result.trim() == 'notchange') {
//                        alert('Offer color already added.')
//                    } else
//                    {
//                        $(that).attr('style', 'background-color:' + result.trim() + ' !important');
//                        $('#editslotModal').modal('hide');
//                        $(that).find('div.peop span').text(discount + '% off');
//                    }
//
//                }
//            });
//        } else {
//            alert('Offers should be 10% and above in order to be active.');
//            return false;
//        }
//    });
</script>
